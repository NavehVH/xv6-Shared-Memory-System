...
{
  //ensures that the region we are mapping covers full pages
  uint64 start = PGROUNDDOWN(src_va); 
  uint64 end = PGROUNDUP(src_va + size); 
  uint64 offset = src_va - start; 

  uint64 dst_start = PGROUNDUP(dst_proc->sz); // ensures we don’t overwrite existing data
  uint64 dst_va = dst_start;

  //Goes over each page from the source address region
  // maps it to a new location in the destination’s address space
  for (uint64 va = start; va < end; va += PGSIZE, dst_va += PGSIZE) {
    ...
    if (mappages(dst_proc->pagetable, dst_va, PGSIZE, pa, flags) != 0) {
      // if even one mapping fails, we clean up everything we previously mapped
      if (dst_va > dst_start) {
...
{
  //ensures that the region we are unmapping covers full pages
  uint64 start = PGROUNDDOWN(addr);
  uint64 end = PGROUNDUP(addr + size);

  //not unmapping beyond the process's memory size
  if (end > p->sz)
    end = p->sz;
  ...
  for (uint64 va = start; va < end; va += PGSIZE) {
    ...
    uvmunmap(p->pagetable, va, 1, 0); //because the page is shared, and maybe still in use by the other process, dont free physical memory
  }
